...
- There is **no automated test suite**. `npm run lint` is `tsc --noEmit` only. Every shipped feature to date was verified by live manual testing (real Firebase project, real Stripe test-mode purchases) — see the `*-plan.md` files for exactly what was tested and how. If you add a feature, verify it the same way: run the dev server, exercise it as a real user would, don't just make it typecheck.
...
| Support ticket email notifications | `RESEND_API_KEY` + `SUPPORT_NOTIFY_EMAIL` (optional — the ticket loop works without email) |
...
- **`storage.rules` and the newest `firestore.rules`/`firestore.indexes.json` entries are not deployed** to the live Firebase project (no authenticated `firebase login` session exists in this environment). Don't assume a rules file you edit is live — it isn't, until someone with CLI access runs `firebase deploy`. Current workarounds (server-side screenshot upload, in-memory ticket sorting) exist specifically because of this; see `admin-support-hardening-plan.md` Phases 1–3 before "fixing" either.
...
3. No test suite exists — introducing one (Vitest is the natural fit, this is already a Vite project) is a real decision, not a drive-by addition. If you do it, prioritize `server/support.ts`'s access-control paths and `server/billing.ts`'s quota transaction — the highest-value targets per `admin-support-hardening-plan.md` Phase 9.

## Verifying your work

There's no CI and no test suite, so verification means running the app:

```bash
npm run dev
```

then exercising the change in a browser against `http://localhost:47293`. For anything touching Firestore/billing/admin, you need real Firebase + Stripe test-mode credentials configured (see the env var table above) — there's no emulator setup in this repo. `npm run seed:demo` gives you a fully-populated, Pro-comped account to test against without needing your own data. `npm run lint` (`tsc --noEmit`) catches type errors but proves nothing about runtime behavior — don't report a change as verified based on lint passing alone.
